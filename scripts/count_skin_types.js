...
let blueCounter = 0, nonBlueCounter = 0, textureCounter = 0;
const blueSkins = {
    female: ["Blue Green", "Purple Blue", "Rose Purple"],
    male: ["Blue Gray", "Blue Green", "Purple Blue", "Rose Purple"],
    child: ["Purple Blue", "Purple", "Blue"]
}
const skinTextures = ["Ethereal", "Fur", "Lava", "Diamond", "Plasmic", "Scales", "Wrinkled"]


const count_skin_types = (type) => {
    const metadata = JSON.parse(fs.readFileSync(`./output/${type}/metadata.json`, { encoding: "utf8" }));
    for(const nft of metadata) {
        if(nft.attributes.some(({trait_type, value}) => trait_type === "Skin" && blueSkins[type].includes(value))) {
            blueCounter += 1;
        } else if(nft.attributes.some(({trait_type, value}) => trait_type === "Skin" && skinTextures.includes(value))) {
            textureCounter += 1;
        } else {
            nonBlueCounter += 1;
        }
